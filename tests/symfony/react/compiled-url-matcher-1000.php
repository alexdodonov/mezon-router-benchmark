...
$benchmark->registerTest(function () {
    ...
    for ($i = 0; $i < \Mezon\Benchmark\Base::$iterationsAmount; $i ++) {
        $staticMatcher->match('/static/' . rand(0, 1000 - 1))['controller']();
    }
    return microtime(true) - $startTime;
}, "[symfony/react/compiled-url-matcher/1000 routes] Resolving static routes %f per second\r\n");
...
$benchmark->registerTest(function () {
    ...
    $startTime = microtime(true);
    for ($i = 0; $i < \Mezon\Benchmark\Base::$iterationsAmount; $i ++) {
        $paramMatcher->match('/param/' . rand(0, 1000 - 1) . '/1')['controller']();
    }
    return microtime(true) - $startTime;
}, "[symfony/react/compiled-url-matcher/1000 routes] Resolving param. routes %f per second\r\n");
